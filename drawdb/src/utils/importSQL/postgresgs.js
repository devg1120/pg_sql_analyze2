...
export function fromPostgresGs(ast, diagramDb = DB.GENERIC) {
  const tables = [];
  const relationships = [];
  const types = [];
  const enums = [];
    ...
    if (e.type === "create") {
      ...
      e.expr.forEach((expr) => {
        if (
          ...
          let endFieldId = tables[endTableId].fields.findIndex(
            (f) => f.name === endField,
          );
          if (endField == -1) return;

          let startFieldId = tables[startTableId].fields.findIndex(
            (f) => f.name === startField,
          );
          if (startFieldId === -1) return;
